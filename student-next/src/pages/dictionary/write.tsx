...
export default function DictionaryWritePage() {
  ...
  const check = async () => {
    ...
    const target = normalize(word.translationRu || "");
    const given = normalize(answer);
    const distance = levenshtein(given, target);
    const ok = distance <= 1;
    setResult(ok ? "success" : "error");
    await submitResult(Number(word.id), "write", ok);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 px-4 py-6 text-white md:px-8 md:py-10">
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        ...
        <h1 className="text-center text-3xl font-bold md:text-4xl">Мини-игры словаря — Напиши перевод</h1>
                ...
                <div className="mx-auto max-w-xl rounded-2xl bg-slate-900 px-6 py-8 text-center shadow-inner">
                  ...
                  <div className="mt-6 flex flex-col items-center gap-3">
                    ...
                    {result && (
                      ...
                      >
                        {result === "success" ? "Верно! Продолжай." : "Проверьте перевод и попробуйте снова."}
                      </div>
                    )}
                  </div>
                </div>
